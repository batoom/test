...
const struct sensors_module_t HAL_MODULE_INFO_SYM = {
    .common = {
        ...
        .name = "kxtf9 & hmc5883 & isl29018 Sensors Module",
...
#define LS_DEVICE_NAME      "/dev/input/event2"
...
static int open_inputs(int mode, int *acc_fd, int *ori_fd, int *l_fd)
{
    ...
    *acc_fd = *ori_fd = -1;
    while((de = readdir(dir))) {
		...
		if (!strcmp(name, "accelerometer_tegra")) {
...
static native_handle_t* control__open_data_source(struct sensors_control_context_t *dev)
{
    ...
    int acc_fd, ori_fd, l_fd;

    if (open_inputs(O_RDONLY, &acc_fd, &ori_fd, &l_fd) < 0 ||
            acc_fd < 0 || ori_fd < 0 || l_fd < 0) {
        return NULL;
    }
...
static int control__wake(struct sensors_control_context_t *dev)
{
    int err = 0;
    int acc_fd, ori_fd, l_fd;
    if (open_inputs(O_RDWR, &acc_fd, &ori_fd, &l_fd) < 0 ||
            acc_fd < 0 || ori_fd < 0 || l_fd < 0) {
        return -1;
    }
...
{ 
	...
	n = select(__MAX(acc_fd, __MAX(ori_fd, ls_fd)) + 1, &rfds,
                   NULL, NULL, NULL);
        ...
        if (FD_ISSET(ori_fd, &rfds)) {
            ...
            if (nread == sizeof(mrawData)) {
                ...
                if (got_syn) {
                    ...
                    data__poll_process_syn_not_inputdev(dev, new_sensors);
                    new_sensors = 0;
                }
            }
